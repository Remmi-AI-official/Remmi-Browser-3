...
object TorStatusChecker {
  ...
  const val TOR_VERIFY_CONNECT_TIMEOUT_SEC = 12L
  const val TOR_VERIFY_READ_TIMEOUT_SEC = 12L
  const val TOR_VERIFY_TOTAL_TIMEOUT_MS = 35_000L
  ...
  private fun executePrimaryCheck(socksPort: Int, startTime: Long, attempt: Int): TorStatusResult? {
    var call: Call? = null
    try {
      val proxy = Proxy(Proxy.Type.SOCKS, InetSocketAddress(InetAddress.getByName("127.0.0.1"), socksPort))
