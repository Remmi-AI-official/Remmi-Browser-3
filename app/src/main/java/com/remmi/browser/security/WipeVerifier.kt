...
object WipeVerifier {
  ...
  suspend fun performLogicalDestructionVerification(
    ...
    try {
      ...
      // Collect any remaining user downloads
      collectDirectoryFiles(extDownloadDir, uniqueResidualFiles)

      // Recursively scan private files and caches for suspicious leftovers (.wal, .shm, .tmp, .bak, etc.)
  ...
  fun classifyGeckoArtifact(file: File): GeckoArtifactClassification {
    ...
    // 2. ENGINE_INFRASTRUCTURE: Required Gecko runtime state & default NSS security databases
    ...
    return GeckoArtifactClassification.ENGINE_INFRASTRUCTURE
  }
