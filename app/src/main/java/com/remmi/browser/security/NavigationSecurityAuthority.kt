...
object NavigationSecurityAuthority {
  ...
  fun isAdOrSpamDestination(rawUrl: String): Boolean {
    ...
    // NOTE: blanket TLD blocks (.xyz, .top, .click, .shop, ...) were removed because
    // they are now widely used by legitimate websites and caused whole sites to fail.
    // We only flag cheap TLDs when they carry MULTIPLE simultaneous tracking params,
    // which is a strong ad-network/redirect signature rather than a normal site visit.
    val hasCheapTld = lower.contains(".cfd/") || lower.contains(".buzz/") || lower.contains(".tk/")
    val trackingParamCount = listOf("cuid=", "cuid/", "/gd/", "token=", "subid=", "zone=", "clickid=").count { lower.contains(it) }
