...
object RedirectInspector {
   ...
   * Rejects dangerous/privileged schemes: javascript:, file:, content:, chrome:, resource:, about:, intent:, filesystem:
   */
  fun isSchemeSafeForNavigation(url: String): Boolean {
    val clean = url.trim().lowercase()
    if (clean.startsWith("javascript:") || clean.startsWith("file:") ||
      clean.startsWith("content:") || clean.startsWith("chrome:") || clean.startsWith("resource:") ||
      clean.startsWith("intent:") || clean.startsWith("filesystem:") ||
      clean.startsWith("blob:") || clean.startsWith("data:")
    ) {
      return false
    }
