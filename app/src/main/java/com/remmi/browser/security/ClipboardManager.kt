  ...
  suspend fun copyImageDirect(imageUrl: String): Boolean = withContext(Dispatchers.IO) {
    try {
      ...
      if (cleanUrl.startsWith("data:image")) {
        ...
        val isGhost = CurrentTorRoute.isGhostActive || NetworkRouteAuthority.isOnionDestination(cleanUrl)
        val client = NetworkRouteAuthority.createHttpClient(
          isGhost = isGhost,
          targetUrl = cleanUrl,
          connectTimeoutSeconds = 8L,
          readTimeoutSeconds = 12L
        )
        ...
        if (!response.isSuccessful) {
          response.close()
          Log.w("ClipboardManager", "Image download failed with status ${response.code}")
