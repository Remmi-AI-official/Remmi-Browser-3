...
object CrashHandlerHelper {
  ...
  private val isInstalled = java.util.concurrent.atomic.AtomicBoolean(false)
  ...
  fun onProcessStart(context: Context) {
    try {
      ...
      // this indicates an abnormal process termination (native crash / OOM / force kill / ANR).
      // Filter out normal user-initiated terminations (like swiping away the task card in Recent Apps).
      if (!wasClean && prevSessionId != null && !hasPendingJavaCrash) {
        val historicalExit = queryHistoricalProcessExit(context)
        val isUserRequested = historicalExit?.classification == ProcessExitClassification.USER_REQUESTED_TERMINATION
        ...
        if (!isUserRequested) {
          ...
          // Persist diagnostics privately. Nothing is written to public/shared storage automatically.
          val privatePath = persistReportPrivately(context, abnormalReport, reportTimestamp, ReportType.ABNORMAL_TERMINATION)
          if (privatePath != null) {
            prefs.edit()
              .putBoolean(KEY_PENDING_EXPORT_CONFIRMED, true)
              .putString(KEY_PENDING_SAVED_PATH, privatePath)
              .apply()
          }
        }
      }
    ...
    if (isInstalled.compareAndSet(false, true)) {
      ...
      Thread.setDefaultUncaughtExceptionHandler { thread, throwable ->
        ...
        try {
          ...
          // Never dump the complete diagnostic report to logcat: it contains device and navigation context.
          Log.e(TAG, "FATAL UNCAUGHT EXCEPTION: ${throwable.javaClass.name}: ${throwable.message}")
          ...
          // 3. Persist a private copy for next-launch diagnostics. Never auto-export to public storage.
          try {
            val privatePath = persistReportPrivately(app, report, now, ReportType.JAVA_CRASH)
            if (privatePath != null) {
              HangWatchdog.recordUiThreadBlockingOpIfNeeded("synchronous SharedPreferences.commit") {
                prefs.edit()
                  .putBoolean(KEY_PENDING_EXPORT_CONFIRMED, true)
  ...
  /**
   * Checks for a pending diagnostic report, restores/ensures its app-private persistence,
   * and consumes the pending marker. No public/shared-storage export is performed.
   */
  fun checkAndExportPendingReport(context: Context): CrashExportResult? {
    try {
      ...
      // If the report was a benign user-initiated task kill (swiping away app), discard it silently
      if (report.contains("Classification: USER_REQUESTED_TERMINATION") ||
          report.contains("Exit Reason: [REMOVE TASK]") ||
          report.contains("reason=USER_REQUESTED") ||
          report.contains("remove-task")
      ) {
        prefs.edit()
      ...
      // If the crash occurred before the private file was persisted, create it now.
      if (!alreadyConfirmed || savedPath == null) {
        savedPath = persistReportPrivately(context, report, timestamp, reportType)
        if (savedPath != null) {
          prefs.edit()
            .putBoolean(KEY_PENDING_EXPORT_CONFIRMED, true)
      ...
      // Only clear pending report marker once private persistence is confirmed.
      ...
      return CrashExportResult(
        fullReport = report,
        savedPath = savedPath ?: "App-private diagnostics (not exported)",
        timestamp = timestamp,
  ...
  /**
   * Persists a diagnostic report inside app-private, no-backup storage.
   * This is deliberately not public/shared storage because reports can include
   * navigation hosts, device metadata, and crash context. Sharing/export is user-initiated.
   */
  private fun persistReportPrivately(
    context: Context,
    report: String,
    timestamp: Long,
    reportType: ReportType = ReportType.JAVA_CRASH
  ): String? {
    return HangWatchdog.recordUiThreadBlockingOpIfNeeded("synchronous private diagnostic write") {
      try {
        ...
        val directory = File(context.noBackupFilesDir, "Remmi Crash Reports").apply { mkdirs() }
        val reportFile = File(directory, fileName)
        val latestFile = File(directory, latestFileName)
        reportFile.writeText(report, Charsets.UTF_8)
        latestFile.writeText(report, Charsets.UTF_8)
        directory.listFiles()
          ?.filter { it.isFile && it.name.endsWith(".txt") && it.name != latestFileName }
          ?.sortedByDescending { it.lastModified() }
          ?.drop(5)
          ?.forEach { runCatching { it.delete() } }
        reportFile.absolutePath
      } catch (e: Throwable) {
        Log.e(TAG, "Failed to persist private diagnostic report: ${e.message}")
        null
      }
    }
  }
  ...
  fun buildDiagnosticReport(
    ...
    val versionName = packageInfo?.versionName ?: "1.0.2"
