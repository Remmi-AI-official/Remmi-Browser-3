...
class GeckoPreferenceController(private val runtime: GeckoRuntime?) {
  companion object {
    ...
    private val runtimeCaches = java.util.Collections.synchronizedMap(
      java.util.WeakHashMap<GeckoRuntime, java.util.concurrent.ConcurrentHashMap<String, Any>>()
    )
    private val fallbackCache = java.util.concurrent.ConcurrentHashMap<String, Any>()

    fun resetCache(targetRuntime: GeckoRuntime? = null) {
      if (targetRuntime != null) {
        runtimeCaches[targetRuntime]?.clear()
        runtimeCaches.remove(targetRuntime)
      } else {
        runtimeCaches.values.forEach { it.clear() }
        runtimeCaches.clear()
        fallbackCache.clear()
      }
    }
  ...
  private val appliedPrefsCache: java.util.concurrent.ConcurrentHashMap<String, Any>
    get() {
      val rt = runtime ?: return fallbackCache
      return runtimeCaches.getOrPut(rt) { java.util.concurrent.ConcurrentHashMap<String, Any>() }
    }
